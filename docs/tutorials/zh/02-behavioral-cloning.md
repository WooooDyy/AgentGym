...
为使模型适应 Agent 相关任务，本文介绍如何使用 AgentGYM 框架对模型进行行为克隆。

本框架是通用智能体的微调平台，目前支持采用 Llama 2 Chat 模板的模型的训练。

## 准备数据集

AgentGYM 提供了各个环境的训练集/验证集 id 划分，你可以从 [Hugging Face](https://huggingface.co/AgentGym) 获取我们的数据集。数据集的格式类似：
